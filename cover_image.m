...
% savedir = "Results/3DOF_tuning/figures/";
% addpath('Results\3DOF_tuning\data_parallel\')
% matfiles = dir('Results\3DOF_tuning\data_parallel\*.mat');
savedir = "Results/cover_image/figures/";
addpath('Results\cover_image\data_parallel\')
matfiles = dir('Results\cover_image\data_parallel\*.mat');
...
%     runs(i).Ki = str2num(K(2));
    J = replace(temp.short_name, 'J', '');
    runs(i).J = str2double(J);
end
...
%          'Color', run.color)
    ...
    t = run.v_reel.Time(run.first_traction_idx);
    t0 = t(1);
    tf = t(end);
    t = linspace(t0, tf, 1e6);
    vr = resample(run.v_reel, t, 'linear');
    Ft = resample(run.TetherForce, t, 'linear');

    size = 0.01 + 50 * (1 + sin(4 * t + 1));
%     size = abs(diff(Ft.Data) ./ diff(vr.Data));
%     size = min(size, 1e7);
%     size = size ./ 1e7 * 50 + 0.1;
%     size = [0.1;size];
    scatter(vr.Data, Ft.Data, size, 'filled')

%     size = 100*(1+sin(10*run.v_reel.Time(run.first_traction_idx)));
%     scatter(run.v_reel.Data(run.first_traction_idx), ...
%             run.TetherForce.Data(run.first_traction_idx), ...
%             size, 'filled')
end
...
% Adjust this graph to make it my thesis report title page.
legend off
xlabel('')
ylabel('')
f = gcf;
...
yticklabels('');
scale = 0.8;
f.Position = [1, 2, 21*scale, 29.7*scale];  % [left bottom width height]
% set(gca, 'YScale', 'log')
% set(gca, 'XScale', 'log')
xmax = 9;
xlim([0, xmax])
...
set(gca,'Visible', 'on')
greyscale = 0.25;
set(gca,'Color',[greyscale, greyscale, greyscale])
hold on
plot(v_ro, Ft_ref_massless, '--w', 'LineWidth', 2);

saveas(gcf, "Results/cover_image/cover_image.png")
...
T = table([runs.Kp]', [runs.Ki]', [runs.Ft_overshoot]', [runs.P_bar_mech_traction]' ./1e6, [runs.P_max]' ./1e6, ...
