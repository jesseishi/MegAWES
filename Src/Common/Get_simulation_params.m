...
%------------- BEGIN CODE --------------
    ...
    if Kite_DOF == 3
        ...
        params.winch_Ft_max=1.192e6;  % 1.192e6 if use_Zgraggen=true, 1.384 if use_Zgraggen=false.
        params.kp_traction_winch=2.5;%0.2;
        params.ki_traction_winch=0.8;%0.4;
    
        % For the kite tether force controller, not used if
        % use_kite_tether_force_control=false.
        params.kite_Ft_max=1.425e6;  % 1.425e6 if use_Zgraggen=true, 1.251e6 if use_Zgraggen=false.
        params.kp_kite_Ft=1.0;  % 1.0 if use_Zgraggen=true, 0.25 if use_Zgraggen=false.
        params.ki_kite_Ft=1.0;  % 1.5 if use_Zgraggen=true, 0.25 if use_Zgraggen=false.
        ...
        % Direct lift control.
        DE2019.CL_DLC_alpha = 1.8/pi;    
        act.dlc.w0 = 75;%35;
        act.dlc.rl = 4;%2;
        act.dlc.max_deflection = deg2rad(30);
        act.dlc.min_deflection = deg2rad(-30);
        act.dlc.bias = 0;
    end
%------------- END CODE --------------
end